# aletheia_gui.py - Fixed Coordinates & Quest System
import pygame
import math
import random
import time
...
# Used for fallback scaling calculations
CAMERA_WIDTH, CAMERA_HEIGHT = 1280, 720
YOLO_SIZE = 640  # The internal resolution of the AI model
BLACK = (0, 0, 0)
...
# =========================
# Quest Manager (With Coordinate Fix)
# =========================
class QuestManager:
    """
    Manages Quests: Red Fog -> Offer -> Timer -> Completion.
    Includes Coordinate Un-Padding to fix the "Left Shift" issue.
    """
    __slots__ = (
    ...
    def __init__(self, shared_state, state_lock):
        ...
        # Tracking list for smooth movement
        self.tracked_targets = []
        
        self.carbon_table = {
            "high": 500.0, "medium": 150.0, "low": 10.0
        }

    def _is_clicked(self, cursor, is_pinching):
        # Rising edge detection for clicks
        return is_pinching and not self.was_pinching

    def _map_coords(self, box, screen_w, screen_h):
        """
        Converts YOLO (640x640 with padding) -> Screen Coordinates.
        Fixes the 'Red Fog on Left' bug.
        """
        # 1. Unpack YOLO box (usually 0-640 range)
        x1, y1, x2, y2 = box
        raw_cx = (x1 + x2) / 2
        raw_cy = (y1 + y2) / 2
        
        # 2. Calculate Letterbox Padding (Aspect Ratio Match)
        # YOLO scales the longest side (1280) to 640. Scale = 0.5.
        scale = YOLO_SIZE / max(CAMERA_WIDTH, CAMERA_HEIGHT)
        
        # Calculate how much height was occupied by the image in the 640 box
        img_h_in_yolo = CAMERA_HEIGHT * scale  # 720 * 0.5 = 360
        pad_y = (YOLO_SIZE - img_h_in_yolo) / 2  # (640 - 360) / 2 = 140
        
        # 3. Un-Pad and Un-Scale
        # X is easy (width matches)
        norm_x = raw_cx / YOLO_SIZE  
        
        # Y needs padding removal
        norm_y = (raw_cy - pad_y) / img_h_in_yolo
        
        # 4. Map to Screen
        final_x = int(norm_x * screen_w)
        final_y = int(norm_y * screen_h)
        
        return final_x, final_y

    def _update_tracked_targets(self, detections, sw, sh, dt):
        """Matches detections to fog clouds to prevent flickering."""
        now = time.time()
        ...
        for det in detections:
            ...
            if best_match:
                # LERP for smoothness (slides the fog)
    ...
    def draw(self, screen, state_snapshot, dt=0.016):
        ...
        # --- ACTIVE TIMER STATE ---
            ...
            res = self._draw_popup(screen,
                                   title=f"Challenge Active: {remaining:.1f}s",
